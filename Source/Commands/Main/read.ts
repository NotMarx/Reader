...
export const command: Command = {
    ...
    run: async (client, message, args, guildLanguage) => {
        ...
        api.getBook(parseInt(args[0])).then(async (res) => {
            ...
            const uploadedAt = `\`${moment(res.uploaded).format("On dddd, MMMM Do, YYYY h:mm A")}\``;
            ...
            message.channel.createMessage({ content: err, messageReference: { messageID: message.id }});
        });
    }
};
