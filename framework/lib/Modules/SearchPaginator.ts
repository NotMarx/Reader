...
export class SearchPaginator {
     ...
     */
    constructor(client: ReaderClient, search: Search, interaction: CommandInteraction<TextableChannel>) {
                        ...
                        this.api.search(this.search.query, parseInt(embed.title.split("Page")[1].split("/")[0]) + 1).then((search) => {
                            ...
                            const embeds = this.search.books.map((book, index) => {
                                ...
                                return new Utils.RichEmbed()
                                    ...
                                    .setDescription(title.join("\n").replace(`${index + 1}. [\`${book.id}\`](https://nhentai.net/g/${book.id}) - \`${book.title.pretty}\``, `**${index + 1}**. **[\`${book.id}\`](https://nhentai.net/g/${book.id})** - **\`${book.title.pretty}\`**`))
                                    .setFooter(`⭐ ${book.favorites.toLocaleString()}`)
                                    .setTitle(this.client.translate("main.page", { firstIndex: this.search.pages.toLocaleString(), lastIndex: this.search.pages.toLocaleString() }))
                                    .setThumbnail(this.api.getImageURL(book.cover))
                                    .addField(this.client.translate("main.title"), `\`${book.title.pretty}\``)
                                    .addField(this.client.translate("main.pages"), `\`${book.pages.length}\``)
                                    .addField(this.client.translate("main.released"), `\`${this.client.translate("main.date", { date: uploadedAt })}\``)
...
export async function createSearchPaginator(client: ReaderClient, search: Search, interaction: CommandInteraction<TextableChannel>) {
